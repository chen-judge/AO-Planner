...
def colorize_draw_agent_and_fit_to_height(
    ...
    if vis_info is not None:
        ...
        if 'predict_ghost' in vis_info:
            maps.draw_waypoint(top_down_map, vis_info['predict_ghost'][[0, 2]], info["meters_per_px"], info["bounds"],
                               maps.PREDICT_GHOST)
...
def navigator_video_frame(
    ...
    rgb = {k: v for k, v in rgb.items() if "_" in k}
    ...
    rgb = [
              add_id_on_img(rgb[i], str(i))
              for i in range(len(rgb))
          ][::-1]

    rgb = np.concatenate([rgb[8], rgb[11], rgb[2], rgb[5]], axis=1).astype(
        np.uint8)  # left(90), front(0), right(270), back(180)
    new_height = int((frame_width / rgb.shape[1]) * rgb.shape[0])
    rgb = cv2.resize(
        rgb,
        (frame_width, new_height),
...
def generate_video(
    ...
    if "disk" in video_option:
        ...
        print(f'images to video in {video_dir} {video_name}...')
