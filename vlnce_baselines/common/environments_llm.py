...
@baseline_registry.register_env(name="VLNCELLMEnv")
class VLNCEDaggerEnv(habitat.RLEnv):
    ...
    def multi_step_control(self, path, tryout, vis_info):
        ...
        for vp_pos in path:
            # visited node includes (vp_id, vp_pos) while ghost node only includes vp_pos
            if len(vp_pos) == 2:
                vp_id, vp_pos = vp_pos
    ...
    def step(self, action, vis_info, *args, **kwargs):
        ...
        if act == 4: # high to low
            ...
            # print("action['back_path']", action['back_path'])
            # 1. back to front node
            if action['back_path'] is None:
                self.teleport(action['front_pos'])
            else:
                # if action['back_path'] is [], skip this step, no need to backtrack, and directly forward to ghost node
            ...
            if action['back_path'] is None:
                ...
                self.multi_step_control(action['local_path_planning'], action['tryout'], vis_info)
        ...
        if self.video_option and done:
            ...
            self.video_dir = os.path.join(self.config.LLM_DIR, f"{year}-{month}-{day}-{hour}-{minute}-{second}-"+"-".join(metric_strs))
