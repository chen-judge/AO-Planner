...
class ETP(Net):
    def __init__(
        ...
        self.rgb_encoder_waypoint = TorchVisionResNet50(
                    observation_space,
    ...
    def forward(self, mode=None, 
                ...
                llm_waypoint_path_cands_batch=None):
        ...
        if mode == 'language':
            ...
            rgb_embedding_waypoint = self.rgb_encoder_waypoint(obs_view12)

            ''' waypoint prediction ----------------------------- '''
            waypoint_heatmap_logits = waypoint_predictor(
                rgb_embedding_waypoint, depth_embedding)
            ...
            for j in range(batch_size):
                ...
                angle_rad_cc = [waypoint["angle"] for waypoint in waypoints][::-1]  # need to inverse to align with waypoint model
                ...
                img_idxes[img_idxes == 12] = 0  # if angle_rad_cc < 0, then index < 0, but still can acquire correct element
                cand_img_idxes.append(img_idxes)
