...
class GraphMap(object):
    ...
    def _localize(self, qpos, kpos_dict, ignore_height=False):
        # find nearest node in kpos_dict for qpos
        min_dis = 10000
        min_vp = None
    ...
    def identify_node(self, cur_pos, cur_ori, cand_ang, cand_dis, cand_pos=False):
        # assume no repeated node
        # since action is restricted to ghosts
        cur_vp = str(len(self.node_pos))

        if cand_pos:  # result from LLM
            cand_vp = [f'{cur_vp}_{str(i)}' for i in range(len(cand_pos))]

        else:  # result from waypoint model
    ...
    def update_graph(self, prev_vp, step_id,
        ...
        if prev_vp is not None:
            ...
            # self.graph_nx.add_edge(prev_vp, cur_vp, weight=dis, path=[np.array([1, 2, 4]), np.array([1, 0, 2])])
            # print(f'prev_vp, cur_vp, dis', prev_vp, cur_vp, dis, self.graph_nx.adj[prev_vp][cur_vp])
        ...
        for i, (cvp, cpos, cembeds) in enumerate(zip(cand_vp, cand_pos, cand_embeds)):
            ...
            # Refer to page 5 of the paper, a visited node is localized
            ...
            # cand not overlap with node, create/update ghost
                ...
                if self.merge_ghost:
                    ...
                    # create ghost, take the input waypoint as a new ghost node
                    ...
                    # update ghost, a ghost node is localized, accumulate the waypoint to the localized ghost node
                    else:
                        gvp = localized_gvp
...
class ZeroShotGraphMap(object):
    ...
    def update_graph(self, prev_vp, step_id,
        ...
        # self.node_embeds[cur_vp] = cur_embeds
        self.node_stepId[cur_vp] = step_id
        for i, (cvp, cpos) in enumerate(zip(cand_vp, cand_pos)):
